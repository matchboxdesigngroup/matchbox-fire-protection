...
namespace MatchboxFireProtection\PostPasswords;
...
class PostPasswords {
	...
	public function password_protection_setting() {
		...
		register_setting( 'writing', 'matchbox_password_protect', $settings_args );

		add_settings_field(
			'matchbox_password_protect',
			esc_html__( 'Enable Password Protected Content', 'matchbox' ),
			[ $this, 'settings_ui' ],
			'writing',
			'default',
			array(
	...
	public function settings_ui() {
		$password_protect = get_option( 'matchbox_password_protect', 0 );

		?>
		<fieldset>
			<legend class="screen-reader-text"><?php esc_html_e( 'Password Protected Content', 'matchbox' ); ?></legend>
			<p>
				<input id="password-protect" name="matchbox_password_protect" type="checkbox" value="1" <?php checked( $password_protect, 1 ); ?> />
			</p>
			<p class="description"><?php esc_html_e( 'Enables password protected content. WordPress default password protected post functionality is insecure and does not work with page caching.', 'matchbox' ); ?></p>
		</fieldset>
		<?php
	}
	...
	public function print_admin_css() {
		...
		$password_protect = (bool) get_option( 'matchbox_password_protect', 0 );
