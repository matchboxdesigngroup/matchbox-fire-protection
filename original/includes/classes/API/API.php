...
namespace MatchboxFireProtection\API;
...
class API {
	...
	public function restrict_rest_api( $result ) {
		...
		if ( 'all' === $restrict && ! $this->user_can_access_rest_api() ) {
			return new \WP_Error( 'rest_api_restricted', esc_html__( 'Authentication Required', 'matchbox' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function restrict_rest_api_setting() {
		...
		add_settings_field( 'matchbox_restrict_rest_api', esc_html__( 'REST API Availability', 'matchbox' ), [ $this, 'restrict_rest_api_ui' ], 'reading' );
	}
	...
	public function restrict_rest_api_ui() {
		...
		?>
		<fieldset>
			<legend class="screen-reader-text"><?php esc_html_e( 'REST API Availability', 'matchbox' ); ?></legend>
			<p><label for="restrict-rest-api-all">
				...
				<?php esc_html_e( 'Restrict all access to authenticated users', 'matchbox' ); ?>
			</label></p>
				...
				<?php
					echo wp_kses_post(
						sprintf(
							...
							__( "Restrict access to the <code><a href='%s'>users</a></code> endpoint to authenticated users", 'matchbox' ),
			...
			<p><label for="restrict-rest-api-n">
				<input id="restrict-rest-api-n" name="matchbox_restrict_rest_api" type="radio" value="none"<?php checked( $restrict, 'none' ); ?> />
				<?php esc_html_e( 'Publicly accessible', 'matchbox' ); ?>
			</label></p>
		</fieldset>
		<?php
	}
	...
	/**
	 * Sanitize the `matchbox_restrict_rest_api` setting.
	 *
