...
namespace MatchboxFireProtection\Plugins;
...
class Plugins {
	...
	public function setup() {
		add_filter( 'install_plugins_tabs', [ $this, 'matchbox_plugin_install_link' ] );
		add_filter( 'install_plugins_table_api_args_matchbox', [ $this, 'filter_install_plugin_args' ] );

		/**
		 * Setup Matchbox suggested plugin display table
		 */
		add_action( 'install_plugins_matchbox', 'display_plugins_table' );
	...
	/**
	 * Add Matchbox suggested tab to plugins install screen
	 *
	 * @param array $tabs Array of tabs.
	 * @return mixed
	 */
	public function matchbox_plugin_install_link( $tabs ) {
		$new_tabs = array(
			'matchbox' => esc_html__( 'Matchbox Suggested', 'matchbox' ),
		);
	...
	/**
	 * Filter the arguments passed to plugins_api() for Matchbox suggested page
	 *
	...
	/**
	 * Warn user when installing non-matchbox suggested plugins
	 */
		...
		<div class="notice notice-warning">
			<p>
				<?php
					echo wp_kses_post(
						sprintf(
							// translators: %s is a link to the Matchbox Suggested plugins screen
							__( "Some plugins may affect display, performance, and reliability. Please consider <a href='%s'>Matchbox Suggestions</a> and consult your site team.", 'matchbox' ),
							esc_url( network_admin_url( 'plugin-install.php?tab=matchbox' ) )
						)
					);
				?>
			</p>
	...
	public function plugin_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
		if ( 'matchbox-fire-protection/matchbox-fire-protection.php' !== $plugin_file ) {
			return $plugin_meta;
		}

		$plugin_meta[] = '<a href="' . esc_url( admin_url( 'admin.php?page=matchbox-about' ) ) . '">' . esc_html__( 'Learn more', 'matchbox' ) . '</a>';
		return $plugin_meta;
	}
