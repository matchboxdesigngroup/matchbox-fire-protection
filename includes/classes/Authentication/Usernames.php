...
class Usernames {
	...
	public function prevent_common_username( $user, $username ) {
		...
		if ( in_array( ! $tld, $test_tlds, true ) && in_array( strtolower( trim( $username ) ), $this->reserved_usernames(), true ) ) {
			return new \WP_Error(
				'Auth Error',
				__( 'Please have an administor change your username in order to meet current security measures.', 'matchbox' )
			);
		}
	...
	public function reserved_usernames() {
		...
		return apply_filters( 'matchbox_fire_protection_reserved_usernames', $common_usernames );
	}
}
